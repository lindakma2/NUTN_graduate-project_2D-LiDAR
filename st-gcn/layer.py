...
class StandConvolution(nn.Module):
    def __init__(self, dims, num_classes, dropout):
        ...
        self.conv = nn.Sequential(
                                   nn.Conv2d(dims[0], dims[1], kernel_size=2, stride=2),#, stride=2
                                   ...
                                   nn.ReLU(inplace=True),
                                   #nn.AvgPool2d(3, stride=2),
                                   nn.Conv2d(dims[2], dims[3], kernel_size=3, stride=2),
                                   nn.InstanceNorm2d(dims[3]),
                                   nn.ReLU(inplace=True),
                                   #nn.AvgPool2d(3, stride=2)
                                   ).to(device)
        
        self.fc = nn.Linear(dims[3]*10, num_classes).to(device)
