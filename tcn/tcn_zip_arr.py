# -*- coding: utf-8 -*-
"""
Created on Thu Mar 31 10:18:08 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 12:58:22 2022
...
for lin in range(1,2):
    ...
    yloadpsth='./軌跡數據/abnormal/abnormal'+str(lin)+'.txt'
    y=np.loadtxt(yloadpsth, dtype=float)
    ...
    tcndata_count=0
    pluslin=lin+0
    xypath="./tcndata/test"+str(pluslin)+"tcn_xy.txt"
    
    tcndata = open(xypath, 'w')
    labelpath="./tcndata/test"+str(pluslin)+"tcn_label.txt"
    tcnlabel = open(labelpath, 'w')
        ...
        if (onecircle < 720):
            ...
            if(tcndata_count<15):
                if(human_index[tcndata_count]==-1):
                    ...
                    tcndata.write('\n')
                    #種類標記
                tcnlabel.write(str(0)+' ')
                tcnlabel.write('\n')
            tcndata_count=tcndata_count+1
            
        count = count + 1
    
    while(tcndata_count<15):
        fifty=0
        while (fifty<28):
            tcndata.write(str(0)+' '+str(0)+' ')
            fifty=fifty+1
